...
import vn.com.vnpt.vinaphone.vnptsoftware.qlvbdhcaobang.view.event.PersonReplyChiDaoEvent;
...
public class ReplyChiDaoActivity extends AppCompatActivity implements IChiDaoView, ILoginView {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_reply_chi_dao);
    ...
    private void reply() {
        event = "SEND_CHIDAO";
        chiDaoPresenter.send(new SendChiDaoRequest(thongTinId, "0", personEditChiDaos.get(0)));
    }
                    ...
                    if (tvTieude.getText() != null && !tvTieude.getText().toString().trim().equals("")) {
                        if (selectedFiles != null && selectedFiles.size() > 0) {
                            ...
                            ChiDaoRequest chiDaoRequest = new ChiDaoRequest("", tvTieude.getText().toString().trim(),
                                    tvNoidung.getText().toString().trim(), fileNames, new ArrayList<String>(), parentId, 0);
                            chiDaoPresenter.saveChiDao(chiDaoRequest);
                        }
                    } else {
                ...
                case 2:
                    ...
                    if (empAdd != null && empAdd.size() > 0) {
                        ...
                        if (empAddPerson.size() > 0 || empDelete.size() > 0) {
                            chiDaoPresenter.savePersonChiDao(new SavePersonChiDaoRequest(thongTinId, empAddPerson, empDelete));
                        } else {
                            nextStep3();
                        }
                    } else {
    ...
    public void onSuccess(Object object) {
        ...
        if (event != null && event.equals("GET_PERSON_EDIT")) {
            ...
            EventBus.getDefault().postSticky(new PersonReplyChiDaoEvent(personEditChiDaos));
            ...
            if (personEditChiDaos != null) {
                for (int i = 0; i < personEditChiDaos.size(); i++) {
                    if (!chons.contains(personEditChiDaos.get(i))) {
                        chons.add(personEditChiDaos.get(i));
                    }
                }
            ...
            case SELECT_FILE_RESULT_SUCCESS:
                if (resultCode == RESULT_OK) {
                    ...
                    if (sessionSelectedFiles != null && sessionSelectedFiles.size() > 0) {
                        ...
                        FileChiDaoAdapter fileChiDaoAdapter = new FileChiDaoAdapter(this, R.layout.item_file_chidao_list, fileChiDaos, "REPLY");
    ...
    private void displayPersonGroup() {
        ...
        EventBus.getDefault().postSticky(new ChoiseGroupEvent(groups, "REPLY"));
    ...
    private void displayPerson() {
        ...
        if (personChiDaos != null && personChiDaos.size() > 0) {
            ...
            PersonChiDaoAdapter contactAdapter = new PersonChiDaoAdapter(this, R.layout.item_person_chidao_list, R.layout.item_person_chidao_detail, R.layout.item_person_chidao_list_detail, lstParent, sizes, countUsers, "REPLY");
            int adapterCount = contactAdapter.getCount();
            for (int i = 0; i < adapterCount; i++) {
                View item = contactAdapter.getView(i, null, null);
                PersonChiDaoCheckEvent personChiDaoCheckEvent = EventBus.getDefault().getStickyEvent(PersonChiDaoCheckEvent.class);
    ...
    public void showProgress() {
        if (!ReplyChiDaoActivity.this.isFinishing()) {
