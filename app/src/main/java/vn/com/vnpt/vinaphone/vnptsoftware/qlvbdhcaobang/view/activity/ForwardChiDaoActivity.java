...
public class ForwardChiDaoActivity extends AppCompatActivity implements IChiDaoView, ILoginView {
    ...
    private void getFiles() {
        if (connectionDetector.isConnectingToInternet()) {
            //showProgress();
            event = "GET_FILE";
            chiDaoPresenter.getFileChiDao(chiDaoFlow.getId());
        } else {
            AlertDialogManager.showAlertDialog(this, getString(R.string.NETWORK_TITLE_ERROR), getString(R.string.NO_INTERNET_ERROR), true, AlertDialogManager.ERROR);
        }
    }
                    ...
                    if (tvTieude.getText() != null && !tvTieude.getText().toString().trim().equals("")) {
                        if (selectedFiles != null && selectedFiles.size() > 0) {
                            ...
                            ChiDaoRequest chiDaoRequest = new ChiDaoRequest("", tvTieude.getText().toString().trim(),
                                    tvNoidung.getText().toString().trim(), fileNames, deleteFiles, chiDaoFlow.getId(), 1);
            ...
            case SELECT_FILE_RESULT_SUCCESS:
                if (resultCode == RESULT_OK) {
                    ...
                    if (sessionSelectedFiles != null && sessionSelectedFiles.size() > 0) {
                        ...
                        FileChiDaoAdapter fileChiDaoAdapter = new FileChiDaoAdapter(this, R.layout.item_file_chidao_list, fileChiDaos, "FORWARD");
    ...
    private void fillFiles() {
        ...
        FileChiDaoAdapter fileChiDaoAdapter = new FileChiDaoAdapter(this, R.layout.item_file_chidao_list, fileChiDaoList, "FW");
        int adapterCount = fileChiDaoAdapter.getCount();
        for (int i = 0; i < adapterCount; i++) {
            View item = fileChiDaoAdapter.getView(i, null, null);
            layoutFile.addView(item);
    ...
    private void displayPersonGroup() {
        lst = new ArrayList<>();
        EventBus.getDefault().postSticky(new ChoiseGroupEvent(groups, "FW"));
    ...
    private void displayPerson() {
        ...
        if (personInGroup != null) {
            emails.setEmails(personInGroup);
        }
        ...
        if (personChiDaos != null && personChiDaos.size() > 0) {
            ...
            PersonChiDaoAdapter contactAdapter = new PersonChiDaoAdapter(this, R.layout.item_person_chidao_list, R.layout.item_person_chidao_detail, R.layout.item_person_chidao_list_detail, lstParent, sizes, countUsers, "FW");
            int adapterCount = contactAdapter.getCount();
            for (int i = 0; i < adapterCount; i++) {
                View item = contactAdapter.getView(i, null, null);
                PersonChiDaoCheckEvent personChiDaoCheckEvent = EventBus.getDefault().getStickyEvent(PersonChiDaoCheckEvent.class);
    ...
    public void showProgress() {
        if (ForwardChiDaoActivity.this.isFinishing()) {
            return;
        }
